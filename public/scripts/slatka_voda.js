...
async function displayData(){
    ...
    try{
        ...
        const res = await fetch(`http://localhost:3001/measured/logs?pool=${encodeURIComponent(poolId)}&year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}`)
        ...
        if (data.length == 0){
            ...
            // data formating
            let newData = []
            // sort by date : should be handled in query
            // data.sort((a, b) => new Date(a.measured_time) - new Date(b.measured_time));
